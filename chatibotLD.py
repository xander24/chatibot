...
def enviar1Fram(frame,clienteCam,ind):
...
def camara1Send():
	...
	while(True):
		if CamIni==1:
			Cam,frame=Cap.read()
			CamIni=2
		SendPro=threading.Thread(target=enviar1Fram,args=(frame,clienteCam,1,))
		SendPro.start()
		Cam,frame=Cap.read()
		SendPro.join()

def enviar2Fram(frame,clienteCam,ind):
	imEncoded,buf=cv2.imencode('.jpg',frame)
	ImgSend=pickle.dumps(buf)
	message=struct.pack("Q",len(ImgSend))+struct.pack("Q",ind)+ImgSend
	clienteCam.send(message)
...
def camara2Send():
	...
	while(True):
		if CamIni==1:
			Cam,frame=Cap.read()
			CamIni=2
		SendPro=threading.Thread(target=enviar2Fram,args=(frame,clienteCam,1,))
		SendPro.start()
		Cam,frame=Cap.read()
		SendPro.join()
...
def InicioRobot():
	...
	while(True):
		...
		if comando=="$OAX01":
			...
			camara1Pro.start()
			camara2Pro=multiprocessing.Process(target=camara2Send)
			camara2Pro.start()
